describe("OAuth Configuration", () => {
  ...
  const loadModule = () => {
    return require("../src/oauth-config") as typeof import("../src/oauth-config");
  };

  describe("getOAuthConfig", () => {
    it("should throw when OAUTH_AUTH_SERVER_URL is not set", () => {
      process.argv = [...originalArgv, "--stream"];
      delete process.env.OAUTH_AUTH_SERVER_URL;
      process.env.OAUTH_SCOPES = "octomind:read,octomind:write";
      process.env.SERVER_BASE_URL = "https://mcp.example.com";
      const { getOAuthConfig } = loadModule();
      expect(() => getOAuthConfig()).toThrow(
        "Invalid environment variables (HTTP mode)",
      );
    });

    it("should throw when SERVER_BASE_URL is not set", () => {
      process.argv = [...originalArgv, "--stream"];
    ...
    it("should throw when OAUTH_SCOPES is not set", () => {
      process.argv = [...originalArgv, "--stream"];
      process.env.OAUTH_AUTH_SERVER_URL = "https://auth.example.com";
      process.env.SERVER_BASE_URL = "https://mcp.example.com";
    ...
    it("should work with PropelAuth test URL", () => {
      ...
      expect(config).toEqual({
        jwksUrl: "https://6393987891.propelauthtest.com/.well-known/jwks.json",
        authEndpoint:
          "https://6393987891.propelauthtest.com/oauth/2.1/authorize",
        tokenEndpoint: "https://6393987891.propelauthtest.com/oauth/2.1/token",
  ...
  describe("getProtectedResourceMetadata", () => {
    it("should generate valid RFC 9728 metadata", () => {
      const config = {
        ...
        authEndpoint: "https://auth.example.com/oauth/2.1/authorize",
        tokenEndpoint: "https://auth.example.com/oauth/2.1/token",
        authServerUrl: "https://auth.example.com",
        serverBaseUrl: "https://mcp.example.com",
        scopes: ["read", "write"],
      };

      const { getProtectedResourceMetadata } = loadModule();
      const metadata = getProtectedResourceMetadata(config);

      expect(metadata).toEqual({
        resource: "https://mcp.example.com",
        authorization_servers: ["https://auth.example.com/oauth/2.1"],
  ...
  describe("getWWWAuthenticateHeader", () => {
    it("should generate WWW-Authenticate header without scope", () => {
      ...
      const { getWWWAuthenticateHeader } = loadModule();
